...
def get_settings():
    ...
    try:
        ...
        return render_template('privates/settings/index.html', user=user_data), 200
    
    except Exception as e:
        logger.error(f"Error loading settings for user {user_id}: {str(e)}", exc_info=True)
...
def update_settings():
    ...
    try:
        ...
        current_user = ProfileService.get_user_profile(
            user_id, include_picture=False
        )
        return jsonify({
            'error': str(e),
            'current_data': current_user
        }), 409
        
    except ValidationError as e:
        # Validation failed
        logger.info(
            f"Validation error for user {user_id}: {list(e.errors.keys())}"
        )
        return jsonify({
