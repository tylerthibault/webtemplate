...
def contact():
    ...
    try:
        ...
        if not name:
            return jsonify({"success": False, "message": "Name cannot be empty"}), 422

        if not email:
            return jsonify({"success": False, "message": "Email cannot be empty"}), 422

        if not subject:
            return (
                jsonify({"success": False, "message": "Subject cannot be empty"}),
                422,
            )
        ...
        return (
            jsonify({"success": False, "message": f"Contact form error: {str(e)}"}),
            500,
        )
