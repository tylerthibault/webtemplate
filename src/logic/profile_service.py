    ...
    def update_profile(user_id: int, data: Dict, submitted_updated_at: str) -> Dict:
        ...
        if 'email' in data:
            ...
            if new_email != old_email:
                logger.info(f"User {user_id} changing email from {old_email} to {new_email}")
                email_changed = True
        ...
        if 'profile_picture_data' in data and data['profile_picture_data']:
            try:
                ...
                if is_valid:
                    ...
                    logger.info(f"Profile picture uploaded for user {user_id} ({mime_type})")
            except Exception as e:
                logger.error(f"Error processing profile picture for user {user_id}: {str(e)}")
        ...
        try:
            ...
            db.session.rollback()
            logger.error(f"Database error updating profile for user {user_id}: {str(e)}", exc_info=True)
