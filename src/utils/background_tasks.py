...
def register_commands(app: Flask):
    ...
    def seed_db_command():
        ...
        with app.app_context():
            ...
            click.echo(f"Seeded {len(result['users'])} users and {len(result['messages'])} messages")
...
class SessionCleanupScheduler:
    ...
    def setup_scheduler(app: Flask, interval_minutes: int = 30):
        ...
        try:
            ...
            scheduler.start()
            app.logger.info(f'Session cleanup scheduler started (every {interval_minutes} minutes)')
